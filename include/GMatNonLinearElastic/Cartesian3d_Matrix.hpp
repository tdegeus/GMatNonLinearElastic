...
{
  m_type   = xt::ones<size_t>({m_nelem, m_nip}) * Type::NonLinearElastic;
  m_index  = xt::ones<size_t>({m_nelem, m_nip}) * m_NonLinearElastic.size();
  m_allSet = true;
...
inline xt::xtensor<double,2> Matrix::kappa() const
{
  ...
  for (size_t e = 0; e < m_nelem; ++e) {
    for (size_t q = 0; q < m_nip; ++q) {
      switch (m_type(e,q)) {
        case Type::NonLinearElastic: out(e,q) = m_NonLinearElastic[m_index(e,q)].kappa(); break;
      }
    }
  }
  return out;
...
inline xt::xtensor<double,2> Matrix::sig0() const
{
  ...
  for (size_t e = 0; e < m_nelem; ++e) {
    for (size_t q = 0; q < m_nip; ++q) {
      switch (m_type(e,q)) {
        case Type::NonLinearElastic: out(e,q) = m_NonLinearElastic[m_index(e,q)].sig0(); break;
      }
    }
  }
  return out;
...
inline xt::xtensor<double,2> Matrix::eps0() const
{
  ...
  for (size_t e = 0; e < m_nelem; ++e) {
    for (size_t q = 0; q < m_nip; ++q) {
      switch (m_type(e,q)) {
        case Type::NonLinearElastic: out(e,q) = m_NonLinearElastic[m_index(e,q)].eps0(); break;
      }
    }
  }
  return out;
...
inline xt::xtensor<double,2> Matrix::m() const
{
  ...
  for (size_t e = 0; e < m_nelem; ++e) {
    for (size_t q = 0; q < m_nip; ++q) {
      switch (m_type(e,q)) {
        case Type::NonLinearElastic: out(e,q) = m_NonLinearElastic[m_index(e,q)].m(); break;
      }
    }
  }
  return out;
...
inline void Matrix::setNonLinearElastic(
  ...
  double m)
{
  GMATNONLINEARELASTIC_ASSERT(m_type.shape() == I.shape());
  ...
  m_type = xt::where(xt::equal(I, 1ul), Type::NonLinearElastic, m_type);
  m_index = xt::where(xt::equal(I, 1ul), m_NonLinearElastic.size(), m_index);
  this->checkAllSet();
  m_NonLinearElastic.push_back(NonLinearElastic(kappa, sig0, eps0, m));
}
...
{
  GMATNONLINEARELASTIC_ASSERT(m_allSet);
  GMATNONLINEARELASTIC_ASSERT(a_Eps.shape() ==\
    std::decay_t<decltype(a_Eps)>::shape_type({m_nelem, m_nip, m_ndim, m_ndim}));
  GMATNONLINEARELASTIC_ASSERT(a_Eps.shape() == a_Sig.shape());
  ...
  for (size_t e = 0; e < m_nelem; ++e) {
    for (size_t q = 0; q < m_nip; ++q) {
      ...
      switch (m_type(e,q)) {
        case Type::NonLinearElastic: m_NonLinearElastic[m_index(e,q)].stress(Eps, Sig); break;
      }
    }
  }
}
        ...
        xt::xtensor<double,6>& a_Tangent) const
{
  ...
  GMATNONLINEARELASTIC_ASSERT(a_Eps.shape() == \
    std::decay_t<decltype(a_Eps)>::shape_type({m_nelem, m_nip, m_ndim, m_ndim}));
  GMATNONLINEARELASTIC_ASSERT(a_Eps.shape() == a_Sig.shape());
  GMATNONLINEARELASTIC_ASSERT(a_Tangent.shape() == \
  ...
  for (size_t e = 0; e < m_nelem; ++e) {
    for (size_t q = 0; q < m_nip; ++q) {
      ...
      switch (m_type(e,q)) {
        case Type::NonLinearElastic: m_NonLinearElastic[m_index(e,q)].tangent(Eps, Sig, C); break;
      }
    }
  }
}
